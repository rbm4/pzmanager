...
function init(callback=null) {
    ...
    return g.base_map.init().then(function(b) {
        ...
        // Initialize region overlays (Spring Boot integration)
        if (regions) {
            regions.init();
            updateRegionsButton();
        }
...
// region overlays (from Spring Boot API)
...
function updateRegionsButton() {
    const btn = document.getElementById('regions_btn');
    if (!btn || !regions) return;
    if (regions.isVisible()) {
        btn.classList.add('active');
    } else {
        btn.classList.remove('active');
    }
